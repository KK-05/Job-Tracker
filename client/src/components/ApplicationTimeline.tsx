 ...
 *
 * Optional selection mode (selectable + selectedIds + onToggleSelect)
 * swaps the status pulse-dot for a checkbox and repurposes row clicks
 * as toggles instead of navigation — used by the bulk actions toolbar
 * on the Applications page. Existing call sites (e.g. the dashboard)
 * are unaffected since selectable defaults to false.
 */
          ...
          return (
            ...
            >
              <div className="flex gap-5 pb-6 last:pb-0 group">
                ...
                <div
                  className={`flex-1 bg-[var(--surface)] border rounded-2xl px-5 py-4 flex items-center justify-between gap-4 transition-all duration-200 ${
                    isSelected
                      ? 'border-[var(--sage)]/50 bg-[var(--sage)]/5'
                      : 'border-[var(--border)] group-hover:border-[var(--border-hi)] group-hover:translate-x-1'
                  }`}
                >
